...
import { getDemoVidById, getAvatarVidById } from "../../../../services/dataset.service";
...
const CollapsibleForm: React.FC<CollapsibleFormProps> = ({ number, form_id, dateTime, status_en, status_bm, name, email, text, video_link, avatar_link, user, user_id, video_name, avatar_name, handleSubmit, handleDelete }) => {
    ...
    return (
        <div className="collapsible-container ">
            <div className={`collapsible-content ${isOpen ? "opened" : "not-opened"}`} onClick={closeForm} data-testid={`collapsible-content_${form_id}`}>
                <div className={`collapsible-content-header ${user}`} onClick={toggleOpen}>
                    ...
                    <div className="expand-icon">{isOpen ? <span>&#9650;</span> : <span>&#9660;</span>}</div>
                </div>
                {isOpen && (
                            ...
                            <div className={`content-left ${user}`}>
                                ...
                                <div className="row">
                                    <div className="col-md-6">
                                        {user === "admin" && <>{status_en === "Rejected" && status_bm === "Ditolak" ? <Descriptions items={video_details_with_video_upload} bordered column={1} /> : status_en === "In Progress" && status_bm === "Dalam Proses" ? <Descriptions items={video_details_with_video_upload} bordered column={1} /> : <Descriptions items={video_details_with_information} bordered column={1} />}</>}

                                        {user === "signexpert" && <Descriptions items={video_details_with_information} bordered column={1} />}
                                    </div>
                                </div>
                            </div>
                                                ...
                                                <div className="dataset-button-individual">
                                                    <Button
                                                        ...
                                                        onClick={() => {
                                                            ...
                                                            handleSubmit(form_id, updateData, false, updatedMessage);
                                            ...
                                            <ButtonProcessing
                                                onClick={async () => {
                                                    if (videoInfo) {
                                                        ...
                                                        try {
                                                            const response = await handleSubmit(form_id, updateData, videoInfo, updatedMessage);
