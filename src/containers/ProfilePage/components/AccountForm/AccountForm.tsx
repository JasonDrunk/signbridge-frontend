...
const AccountForm = () => {
    ...
    const [selectedCountryOption, setSelectedCountryOption] = useState(user?.country ?? "");
    const [selectedStateOption, setSelectedStateOption] = useState(user?.state ?? "");
    const [selectedCityOption, setSelectedCityOption] = useState(user?.city ?? "");
    ...
    const uploadInputData = async (url?: string) => {
        ...
        const result = await UpdateUserProfileById(user.firebase_id, data);
        if (result) {
            setUser({
                ...user,
        ...
        <div className={style.accountFormContainer}>
            <div className={style.accountContent}>
                ...
                <form onSubmit={handleSubmit} className={style.form} key={customKey}>
                    <AccountInputField label={t("accountUsername")} name="username" value={formData.username} onChange={handleChange} error={error.username} />
                    <div className={style.inputGroup}>
                        <AccountInputField label={t("accountFirstName")} name="firstName" value={formData.firstName} onChange={handleChange} error={error.firstName} />
                        <AccountInputField label={t("accountLastName")} name="lastName" value={formData.lastName} onChange={handleChange} error={error.lastName} />
                    </div>
                    <div className={style.inputGroup}>
                        <AccountInputField label={t("accountAge")} name="age" value={formData.age} onChange={handleChange} error={error.age} />
                        <div className={style.radio_group}>
                            ...
                            <div className={style.gender}>
                                ...
                                <input type="radio" name="gender" id="female" value="female" className="female" onChange={handleChange} checked={formData.gender === "female"} />
                                <label htmlFor="female"> {t("accountFemale")}</label>
                            </div>
                        </div>
                    </div>
                    <AccountInputField label={t("accountRace")} name="race" value={formData.race} onChange={handleChange} error={error.race} />
                    ...
                    <div className={style.selectContainer}>
                        <Ariakit.SelectProvider
                            ...
                            <Ariakit.SelectLabel className={style.label}>{t("accountCountry")}</Ariakit.SelectLabel>
                            <Ariakit.Select className={style.selectTrigger}>
                                {selectedCountryOption ? selectedCountryOption : t("selectCountry")}
                    ...
                    <div className={style.selectContainer}>
                        <Ariakit.SelectProvider
                            ...
                            <Ariakit.SelectLabel className={style.label}>{t("accountState")}</Ariakit.SelectLabel>
                            <Ariakit.Select className={style.selectTrigger}>
                            ...
                            <Ariakit.SelectPopover gutter={4} sameWidth className={style.selectPopover}>
                                {selectedCountryOption
                                    ? countries
                                          ?.filter(country => country.country === selectedCountryOption)[0]
                    ...
                    <div className={style.selectContainer}>
                        <Ariakit.SelectProvider
                            ...
                            <Ariakit.SelectLabel className={style.label}>{t("accountCity")}</Ariakit.SelectLabel>
                            <Ariakit.Select className={style.selectTrigger}>
                            ...
                            <Ariakit.SelectPopover gutter={4} sameWidth className={style.selectPopover}>
                                {selectedStateOption
                                    ? countries
                                          ?.filter(country => country.country === selectedCountryOption)[0]
                                          .state.filter(state => state.name === selectedStateOption)[0]
                                          .city.map((city, index) => (
                                              <Ariakit.SelectItem
                                                  key={index}
                                                  className={style.selectItem}
                    ...
                    <div className={style.imageUploader}>
                        <label htmlFor="file" className={style.uploadLabel}>
                            {image ? <img src={image ?? ""} alt="Uploaded" className={style.uploadedImage} /> : <span>{t("uploadAccImage")}</span>}
                        </label>
                        <label htmlFor="file" className={style.uploadButton}>
                            {t("selectAccImage")}
                            <input type="file" id="file" className={style.uploadInput} onChange={handleImageChange} style={{ display: "none" }} accept="image/jpeg, image/png, image/jpg" />
                        </label>
                    </div>
