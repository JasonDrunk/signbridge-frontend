		...
		columnHelper.accessor(row => row.status_SE_en, {
			...
			cell: info => (
				<div
					...
					style={
						["In Progress", "Awaiting Accept", "Awaiting Verification", "Verified", "Rejected"].includes(info.getValue())
				...
				<div className={style.signTextTableContainer}>
					...
					<div className={style.pagination}>
						...
						<div className={style.dataNumber}>
							{table.getRowModel().rows.length === 0 ? 0 : table.getRowModel().rows[0].index + 1} -{" "}
							{table.getRowModel().rows.length === 0 ? 0 : table.getRowModel().rows[table.getRowModel().rows.length - 1].index + 1} of{" "}
							{publicTableData.length}
						</div>
