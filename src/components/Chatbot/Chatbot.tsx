        ...
        <div>
            <Popover.Root open={isChatbotOpen} onOpenChange={toggleChatbot}>
                <Popover.Trigger asChild>
                    <motion.div className={styles.circleChatbot} onClick={toggleChatbot} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} transition={{ type: "spring", stiffness: 300, damping: 20 }}>
                        {isChatbotOpen ? <IoIosArrowDown size={25} /> : <IoChatbubbleEllipses size={25} />}
                    </motion.div>
                </Popover.Trigger>
                    ...
                    <AnimatePresence>
                        {isChatbotOpen && (
                            <motion.div
                                ...
                                <div className={styles.response} style={{ whiteSpace: "pre-wrap" }}>
                                    {messages.map((msg, index) => (
                                        <div key={index} className={`${styles.messageContainer} ${msg.role === "user" ? styles.userMessage : styles.botMessage}`}>
                                            ...
                                            <div className={styles.side}>
                                                <p className={styles.role}>{msg.role === "user" ? "You " : "Bot "}</p>
